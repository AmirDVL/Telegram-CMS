...
[`RUNBOOK.md`](RUNBOOK.md#9-monitoring-prometheus--grafana)).

- **Default:** `admin`
- **Auto-generated by:** `install.sh` (64-character hex string)
- **Used by:** grafana container only
- **Required in production:** yes — the installer sets a strong value; if you
  deployed manually, change the default `admin`/`admin` credentials before
  exposing port `3001`
